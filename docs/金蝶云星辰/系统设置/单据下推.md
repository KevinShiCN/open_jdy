---
title: "单据下推 更新于2024-02-29 21:58 复制链接"
product: "金蝶云星辰"
category: "系统设置 > 单据下推"
source: "https://open.jdy.com/#/files/api/detail?index=3&categrayId=3cc8ee9a663e11eda5c84b5d383a2b93&id=9e8be44e712511eda0b323f66a87eddb"
last_update: "2024-02-29 21:58"
crawl_date: "2026-03-03"
---

# 单据下推 更新于2024-02-29 21:58 复制链接

> 产品：金蝶云星辰 | 分类：系统设置 > 单据下推

单据下推 更新于2024-02-29 21:58 复制链接    一级标题 二级标题 三级标题 四级标题 五级标题 六级标题                添加图片链接 上传图片
                                         
```
### 基本信息
- 用途说明：调用单据下推将直接保存新建目标单据并返回单据id
- 请求方式：POST
- 请求地址：https://api.kingdee.com/jdy/v2/sys/bill_push
### 请求参数
#### headers参数
| 参数名称               | 参数示例                        | 是否必填 | 参数说明                                                                                                                                                             |
| --- | --- | --- | --- |
...
| --- | --- | --- | --- |
| id          | string  | true  | 源单id                                                                                                                                                            
| source_bill | string  | true  | 源单标识（采购订单pur_bill_order、采购入库pur_bill_inbound、销售订单sal_bill_order、销售出库sal_bill_outbound、调拨出库单inv_tfout_bill_outbound、采购申请单pur_bill_request、委托代销单sal_bill_entrust） 
| target_bill | string  | true  | 目标单标识（采购入库pur_bill_inbound、付款单ap_paybill、销售出库sal_bill_outbound、收款单ar_creditbill、调拨入库inv_tfin_bill_inbound、采购订单pur_bill_order、委托代销退货单sal_bill_entrust_rtn）       
| merge_push  | boolean | false | true:多个单据id下推后合并成为一个目标单据id，目前仅支持销售订单下推销售出库单，默认false                                                                                                             
...
<ul>
<li>用途说明：调用单据下推将直接保存新建目标单据并返回单据id</li>
<li>请求方式：POST</li>
<li>请求地址：https://api.kingdee.com/jdy/v2/sys/bill_push</li>
</ul>
...
<td>string</td>
<td>true</td>
<td>源单标识（采购订单pur_bill_order、采购入库pur_bill_inbound、销售订单sal_bill_order、销售出库sal_bill_outbound、调拨出库单inv_tfout_bill_outbound、采购申请单pur_bill_request、委托代销单sal_bill_entrust）</td>
</tr>
<tr>
<td>target_bill</td>
<td>string</td>
<td>true</td>
<td>目标单标识（采购入库pur_bill_inbound、付款单ap_paybill、销售出库sal_bill_outbound、收款单ar_creditbill、调拨入库inv_tfin_bill_inbound、采购订单pur_bill_order、委托代销退货单sal_bill_entrust_rtn）</td>
</tr>
<tr>
<td>merge_push</td>
<td>boolean</td>
<td>false</td>
<td>true:多个单据id下推后合并成为一个目标单据id，目前仅支持销售订单下推销售出库单，默认false</td>
</tr>
</tbody>
</table>
<h4><a id="_26"></a>请求示例</h4>
<pre><div class="hljs"><code class="lang-txt">curl --location --request  post &#x27;https://api.kingdee.com/jdy/v2/sys/bill_push&#x27; \
--header &#x27;Content-Type: application/json&#x27; \
--header &#x27;X-Api-ClientID: 205022&#x27; \
--header &#x27;X-Api-Auth-Version: 2.0&#x27; \
--header &#x27;X-Api-TimeStamp: 1655775240000&#x27; \
